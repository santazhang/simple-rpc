...
def forbid_reserved_names(name):
    ...
    if name == "reg_to":
        raise Exception("bad name 'reg_to', it is a reserved name")
...
def emit_service_and_proxy_python(service, f):
    ...
    with f.indent():
        ...
        with f.indent():
            ...
            f.writeln("return f")
        f.writeln()
        f.writeln("def reg_to(self, server):")
        with f.indent():
            for func in service.functions:
                f.writeln("server.reg_func(%sService.%s, self.__bind_helper__(self.%s), [%s], [%s])" % (
